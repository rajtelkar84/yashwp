...
namespace West.EnterpriseUX.Automation.UWP.Utilities
{
    public class Helper
    {
        ...
        public void CheckCustomizableFileExists(string completeFilePath)
        {
            Logger.LogInfo($"The expected file presence in this path {completeFilePath} : {File.Exists(completeFilePath)} ");
            if (!File.Exists(completeFilePath))
            {
                Logger.WriteToCSV(BaseTest.inboxesMemoryUtilizationLogFileName, TestInboxFields.InboxName + "," + TestInboxFields.InboxOutcome + "," + TestInboxFields.InboxStatus + "," + TestInboxFields.ElapsedTime + "," + TestInboxFields.AppMemory + "," + TestInboxFields.Screenshot);
            }
        }
    }
}
